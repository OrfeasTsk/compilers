import java_cup.runtime.*;

%%
/* ----------------- Options and Declarations Section----------------- */

/*
   The name of the class JFlex will create will be Scanner.
   Will write the code to the file Scanner.java.
*/
%class Scanner

/*
  The current line number can be accessed with the variable yyline
  and the current column number with the variable yycolumn.
*/
%line
%column

/*
   Will switch to a CUP compatibility mode to interface with a CUP
   generated parser.
*/
%cup
%unicode

/*
  Declarations

  Code between %{ and %}, both of which must be at the beginning of a
  line, will be copied letter to letter into the lexer class source.
  Here you declare member variables and functions that are used inside
  scanner actions.
*/

%{
    /**
        The following two methods create java_cup.runtime.Symbol objects
    **/
    StringBuffer stringBuffer = new StringBuffer();
    private Symbol symbol(int type) {
       return new Symbol(type, yyline, yycolumn);
    }
    private Symbol symbol(int type, Object value) {
        return new Symbol(type, yyline, yycolumn, value);
    }
%}

/*
  Macro Declarations

  These declarations are regular expressions that will be used latter
  in the Lexical Rules Section.
*/

/* White space is a line terminator, space, tab, or line feed. */
WhiteSpace     = [\r\n\t\f ]

/* In order to create valid words   */
WordCharacters = [a-zA-Z0-9_]



%state STRING

%%
/* ------------------------Lexical Rules Section---------------------- */

<YYINITIAL> {
/* operators */
 "+"                             { return symbol(sym.PLUS); }
 ","                             { return symbol(sym.COMMA); }
 "("                             { return symbol(sym.LPAREN); }
 ")"                             { return symbol(sym.RPAREN); }
 "{"                             { return symbol(sym.LCBR); }
 "}"                             { return symbol(sym.RCBR); }
 "if"                            { return symbol(sym.IF); }
 "else"                          { return symbol(sym.ELSE); }
 "prefix"                        { return symbol(sym.PREFIX); }
 "suffix"                        { return symbol(sym.SUFFIX); }
 \"                              { stringBuffer.setLength(0); stringBuffer.append('"'); yybegin(STRING); }
 [a-zA-Z_]{WordCharacters}{0,30} { return symbol(sym.ID, yytext()); } /* Identifiers can start with a letter (capital or not) or underscore, followed by 30 maximum characters including letters, digits and underscores (C like identifiers)   */
 (\){WhiteSpace}*\{)             { return symbol(sym.RPAREN_LCBR); }  /* Concatenation of ')' and '{' with zero or more white spaces between */
 {WhiteSpace}                    { /* Just skip what was found, do nothing */ }
}

<STRING> {
      \"                             { stringBuffer.append('"');
                                       yybegin(YYINITIAL);
                                       return symbol(sym.STRING_LITERAL, stringBuffer.toString()); }
      [^\r\n\t\f]                    { stringBuffer.append( yytext() ); } /* Everything except escaped characters */
      \\t                            { stringBuffer.append('\t'); }
      \\n                            { stringBuffer.append('\n'); }
      \\r                            { stringBuffer.append('\r'); }
      \\f                            { stringBuffer.append('\f'); }
      \\\"                           { stringBuffer.append('\"'); }
      \\                             { stringBuffer.append('\\'); }
}

/* No token was found for the input so throw an error.  Print out an
   Illegal character message with the illegal character that was found. */
[^]                    { throw new Error("Illegal character <"+yytext()+">"); }
